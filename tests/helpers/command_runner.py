...
def run_script(
    ...
    """
    # Load script mappings from YAML config (NO hardcoded values)
    script_mappings = load_script_mappings()
    ...
    if command is None and args:
        # Load session subcommand mappings from YAML config (NO hardcoded values)
        session_subcommand_map = load_session_subcommands()
        subcommand = args[0]
        command = session_subcommand_map.get(subcommand, subcommand)
        args = args[1:]  # Remove subcommand from args

        # Special handling for "track" command - it requires a nested subcommand
        if command == "track":
    ...
    # Run via the main edison CLI entry point using python -m to avoid PATH issues
    cmd = [sys.executable, "-m", "edison", domain, command] + args
