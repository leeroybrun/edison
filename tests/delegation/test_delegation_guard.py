...
class TestImplementationReportGuard(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # Create session with deterministic ID matching project_OWNER
        run_with_timeout([str(self.session), "new", "--session-id", self.env["project_OWNER"]], cwd=REPO_ROOT, env=self.env, check=True, text=True, capture_output=True)
    ...
    def test_implementation_report_required_for_all_tasks(self) -> None:
        ...
        # tasks/ready should fail because implementation-report.json is missing
