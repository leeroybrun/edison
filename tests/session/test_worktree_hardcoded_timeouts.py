...
@pytest.fixture
def git_repo_with_custom_timeouts(tmp_path, monkeypatch):
    ...
    run_with_timeout(["git", "config", "user.email", "test@example.com"], cwd=repo_dir, check=True)
    run_with_timeout(["git", "config", "user.name", "Test User"], cwd=repo_dir, check=True)

    # Create initial commit with REAL file
    readme = repo_dir / "README.md"
    readme.write_text("# Test Repository\n")
    run_with_timeout(["git", "add", "README.md"], cwd=repo_dir, check=True)
    run_with_timeout(["git", "commit", "-m", "Initial commit"], cwd=repo_dir, check=True)
...
def test_worktree_manager_uses_config_timeouts_not_hardcoded(git_repo_with_custom_timeouts):
    ...
    cfg = SessionConfig(repo_root=git_repo_with_custom_timeouts)
    ...
    result = run_with_timeout(
        ...
        cwd=git_repo_with_custom_timeouts,
        capture_output=True
    )
...
def test_worktree_cleanup_uses_config_timeouts_not_hardcoded(git_repo_with_custom_timeouts):
...
def test_git_worktree_utils_uses_config_timeouts_not_hardcoded(git_repo_with_custom_timeouts):
    ...
    is_healthy_fake = check_worktree_health(git_repo_with_custom_timeouts / "nonexistent")
    ...
    existing_path = get_existing_worktree_path(branch, repo_root=git_repo_with_custom_timeouts)
    assert existing_path is not None
    assert existing_path == wt_path

    # Test get_existing_worktree_path for non-existent branch
    non_existent = get_existing_worktree_path("session/does-not-exist", repo_root=git_repo_with_custom_timeouts)
...
def test_worktree_operations_respect_yaml_config_integration(git_repo_with_custom_timeouts):
...
def test_worktree_dry_run_respects_config(git_repo_with_custom_timeouts):
    ...
    existing = get_existing_worktree_path(branch, repo_root=git_repo_with_custom_timeouts)
    assert existing is None


def test_worktree_idempotent_operations_use_config(git_repo_with_custom_timeouts):
...
def test_concurrent_worktree_operations_use_config(git_repo_with_custom_timeouts):
...
def test_worktree_error_handling_uses_config(git_repo_with_custom_timeouts):
    ...
    non_existent_path = git_repo_with_custom_timeouts / "worktrees" / "does-not-exist"
