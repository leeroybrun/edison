...
def _write(rd: Path, name: str, body: str) -> None:
    p = rd / name
    p.write_text(body)
...
@pytest.mark.fast
def test_only_detection_blocks_ready(test_project_dir: TestProjectDir):
    task_id = "808-w2-only-detect"
    session_id = "sess-only"
    run_script("tasks/new", ["--id", "808", "--wave", "w2", "--slug", "only-detect"], cwd=test_project_dir.tmp_path)
...
@pytest.mark.fast
def test_coverage_script_enforced_at_ready(test_project_dir: TestProjectDir):
    task_id = "809-w2-coverage"
    session_id = "sess-coverage"
    run_script("tasks/new", ["--id", "809", "--wave", "w2", "--slug", "coverage"], cwd=test_project_dir.tmp_path)
    rd = _ensure_ready_prereqs(test_project_dir, task_id, session_id)
    ...
    scripts_dir = test_project_dir.tmp_path / "scripts"
...
@pytest.mark.fast
def test_hmac_tamper_detection(test_project_dir: TestProjectDir, monkeypatch):
    task_id = "810-w2-hmac"
    session_id = "sess-hmac"
    run_script("tasks/new", ["--id", "810", "--wave", "w2", "--slug", "hmac"], cwd=test_project_dir.tmp_path)
    rd = _ensure_ready_prereqs(test_project_dir, task_id, session_id)
    ...
    _ = run_script("tasks/ready", [task_id, "--session", session_id, "--run"], cwd=test_project_dir.tmp_path)
    ...
    res2 = run_script("tasks/ready", [task_id, "--session", session_id], cwd=test_project_dir.tmp_path)
