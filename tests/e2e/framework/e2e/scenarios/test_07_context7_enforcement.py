...
def test_context7_note_bypass_is_rejected(test_project_dir: TestProjectDir):
    ...
    run_script("session", ["new", "--owner", session_id, "--session-id", session_id, "--mode", "start"], cwd=test_project_dir.tmp_path)
    run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug], cwd=test_project_dir.tmp_path)
    run_script("session", ["new", "--owner", "test", "--session-id", session_id, "--mode", "start"], cwd=test_project_dir.tmp_path)
    run_script("tasks/claim", [task_id, "--session", session_id], cwd=test_project_dir.tmp_path)
    ...
    _ensure_impl_validate_ok(test_project_dir.project_root)
    _ensure_base_evidence(test_project_dir.project_root, task_id, 1)
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_marker_file_structure(test_project_dir: TestProjectDir):
    ...
    res_new = run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug], cwd=test_project_dir.tmp_path)
    assert_command_success(res_new)
    res_wip = run_script("tasks/status", [task_id, "--status", "wip"], cwd=test_project_dir.tmp_path)
    assert_command_success(res_wip)

    # Create QA brief (waiting is acceptable for ready checks)
    res_qa = run_script("qa/new", [task_id], cwd=test_project_dir.tmp_path)
    assert_command_success(res_qa)

    # Establish base evidence + round structure
    _ensure_base_evidence(test_project_dir.project_root, task_id, round_num=1)

    # Add Context7 evidence (normalized package name)
    marker = _write_context7(test_project_dir.project_root, task_id, "react", round_num=1, content="React evidence\n")
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_multiple_packages(test_project_dir: TestProjectDir):
    ...
    res_new = run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug], cwd=test_project_dir.tmp_path)
    assert_command_success(res_new)
    run_script("tasks/status", [task_id, "--status", "wip", "--session", session_id], cwd=test_project_dir.tmp_path)
    run_script("qa/new", [task_id], cwd=test_project_dir.tmp_path)
    ...
    for pkg in packages:
        _write_context7(test_project_dir.project_root, task_id, pkg, 1, session_id=session_id)

    ev_dir = _evidence_dir(test_project_dir.project_root, task_id, 1)
    for pkg in packages:
...
def test_context7_detection_from_file_extensions(combined_env):
    ...
    todo = _task_file_path(test_project_dir.project_root, "todo", task_id)
    ...
    if worktree_path and worktree_path.exists():
        ...
        test_git_repo.commit_in_worktree(worktree_path, "feat: add tsx and route")
    else:
        # Fallback: use Primary Files metadata in non-worktree sandbox
        p_todo = _task_file_path(test_project_dir.project_root, "todo", task_id)
        p_wip = _task_file_path(test_project_dir.project_root, "wip", task_id)
    ...
    res_done = run_script("tasks/status", [task_id, "--status", "done"], cwd=test_project_dir.tmp_path, env=env)
    assert_command_failure(res_done)
...
def test_context7_detection_from_imports(combined_env):
    ...
    if wt2 and wt2.exists():
        ...
        test_git_repo.commit_in_worktree(wt2, "feat: add zod schema + react app")
    else:
        # Fallback: declare files in task metadata
        p3_todo = _task_file_path(test_project_dir.project_root, "todo", task_id)
        p3_wip = _task_file_path(test_project_dir.project_root, "wip", task_id)
...
@pytest.mark.context7
@pytest.mark.edge_case
def test_context7_missing_evidence_detection(test_project_dir: TestProjectDir):
...
def test_context7_cross_check_task_metadata_vs_git_diff(combined_env):
    ...
    if worktree_path and worktree_path.exists():
        ...
        test_git_repo.commit_in_worktree(worktree_path, "feat: add zod schema")
    else:
        # Fallback when worktrees are unavailable: declare zod file via metadata
        target = _task_file_path(test_project_dir.project_root, "todo", task_id)
        if not target.exists():
            target = _task_file_path(test_project_dir.project_root, "wip", task_id)
    ...
    _ensure_base_evidence(test_project_dir.project_root, task_id, 1)
    _write_context7(test_project_dir.project_root, task_id, "react", 1, session_id=session_id)

    res = run_script("tasks/status", [task_id, "--status", "done", "--session", session_id], cwd=test_project_dir.tmp_path)
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_evidence_per_round(test_project_dir: TestProjectDir):
    ...
    _ensure_impl_validate_ok(test_project_dir.project_root)
    _ensure_base_evidence(test_project_dir.project_root, task_id, 1)
    _write_context7(test_project_dir.project_root, task_id, "react", 1)

    _ensure_base_evidence(test_project_dir.project_root, task_id, 2)
    _write_context7(test_project_dir.project_root, task_id, "react", 2)
    _write_context7(test_project_dir.project_root, task_id, "zod", 2)

    round1 = _evidence_dir(test_project_dir.project_root, task_id, 1)
    round2 = _evidence_dir(test_project_dir.project_root, task_id, 2)
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_package_name_normalization(test_project_dir: TestProjectDir):
    ...
    _write_context7(test_project_dir.project_root, task_id, "react", 1)
    _write_context7(test_project_dir.project_root, task_id, "next", 1)
...
@pytest.mark.context7
@pytest.mark.edge_case
def test_context7_no_packages_required(test_project_dir: TestProjectDir):
    ...
    assert_file_exists(_task_file_path(test_project_dir.project_root, "done", task_id))
...
def test_context7_complete_enforcement_workflow(combined_env):
    ...
    run_script("tasks/status", [task_id, "--status", "wip"], cwd=test_project_dir.tmp_path)
    run_script("qa/new", [task_id, "--owner", session_id, "--session", session_id], cwd=test_project_dir.tmp_path)
    ...
    if worktree_path and worktree_path.exists():
        ...
        target_meta = _task_file_path(test_project_dir.project_root, "todo", task_id)
        if not target_meta.exists():
            target_meta = _task_file_path(test_project_dir.project_root, "wip", task_id)
    ...
    _write_context7(test_project_dir.project_root, task_id, "react", 1)
    env = {"project_OWNER": session_id}
    res_fail = run_script("tasks/status", [task_id, "--status", "done"], cwd=test_project_dir.tmp_path, env=env)
    ...
    res_ok = run_script("tasks/status", [task_id, "--status", "done"], cwd=test_project_dir.tmp_path, env=env2)
...
def test_context7_prisma_schema_enforcement(combined_env):
    ...
    run_script("tasks/claim", [task_id, "--status", "wip", "--session", session_id], cwd=test_project_dir.tmp_path)
    run_script("qa/new", [task_id, "--owner", session_id, "--session", session_id], cwd=test_project_dir.tmp_path)
...
""".strip()
        ...
        test_git_repo.commit_in_worktree(worktree_path, "feat: add prisma schema")
    else:
    ...
    _write_context7(test_project_dir.project_root, task_id, "prisma", 1, session_id=session_id)
    env2 = dict(env)
    env2["DISABLE_TDD_ENFORCEMENT"] = "1"
    res_ok = run_script("tasks/status", [task_id, "--status", "done"], cwd=test_project_dir.tmp_path, env=env2)
    assert_command_success(res_ok)
...
def test_context7_prisma_migration_enforcement(combined_env):
    ...
    if worktree_path and worktree_path.exists():
        ...
        test_git_repo.commit_in_worktree(worktree_path, "feat: add prisma migration")
    else:
        target_meta = _task_file_path(test_project_dir.project_root, "todo", task_id)
        if not target_meta.exists():
            target_meta = _task_file_path(test_project_dir.project_root, "wip", task_id)
...
def test_context7_prisma_seeds_enforcement(combined_env):
    ...
    if worktree_path and worktree_path.exists():
        ...
        test_git_repo.commit_in_worktree(worktree_path, "feat: add prisma seed")
    else:
        target_meta = _task_file_path(test_project_dir.project_root, "todo", task_id)
        if not target_meta.exists():
            target_meta = _task_file_path(test_project_dir.project_root, "wip", task_id)
...
def test_context7_fails_closed_when_config_missing(
    test_project_dir: TestProjectDir, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    cfg_yaml = test_project_dir.agents_root / "config" / "validators.yml"
    if cfg_yaml.exists():
        cfg_yaml.unlink()

    # Ensure ConfigManager and task resolve the isolated project root.
    monkeypatch.setenv("AGENTS_PROJECT_ROOT", str(test_project_dir.tmp_path))
...
def test_context7_zod_not_triggered_by_route_only(combined_env):
    ...
    if wt and wt.exists():
        ...
        test_git_repo.commit_in_worktree(wt, "feat: add route + tsx without zod")
    else:
        # Fallback: declare files via Primary Files metadata
        path_pf = _task_file_path(test_project_dir.project_root, "todo", task_id)
        if not path_pf.exists():
            path_pf = _task_file_path(test_project_dir.project_root, "wip", task_id)
    ...
    env = {"project_OWNER": session_id}
    res = run_script("tasks/status", [task_id, "--status", "done"], cwd=test_project_dir.tmp_path, env=env)
    assert_command_failure(res)
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_non_git_primary_files_detection(test_project_dir: TestProjectDir):
...
@pytest.mark.context7
@pytest.mark.fast
def test_context7_marker_content_validation(test_project_dir: TestProjectDir):
    ...
    res_fail = run_script("tasks/status", [task_id, "--status", "done", "--session", session_id], cwd=test_project_dir.tmp_path)
    assert_command_failure(res_fail)
    ...
    _write_context7(test_project_dir.project_root, task_id, "react", 1, content=valid, session_id=session_id)
    # Disable TDD for success fast‑path
    env = {"DISABLE_TDD_ENFORCEMENT": "1"}
    res_ok = run_script("tasks/status", [task_id, "--status", "done", "--session", session_id], cwd=test_project_dir.tmp_path, env=env)
    assert_command_success(res_ok)
