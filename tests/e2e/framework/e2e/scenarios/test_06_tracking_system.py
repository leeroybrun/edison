...
@pytest.mark.fast
def test_session_created_timestamp(test_project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_task_tracking_timestamps(test_project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_session_last_active_tracking(test_project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_activity_log_entries(test_project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_continuation_id_tracking(test_project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_continuation_id_end_to_end(test_project_dir: TestProjectDir):
    ...
    assert_command_success(
        run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug, "--session", session_id, "--continuation-id", cid], cwd=test_project_dir.tmp_path)
    )
    # Stamp QA with continuation ID as well
    assert_command_success(
        run_script("qa/new", [task_id, "--session", session_id, "--continuation-id", cid], cwd=test_project_dir.tmp_path)
    )
    ...
    ev = test_project_dir.project_root / "qa" / "validation-evidence" / task_id / "round-1"
    ev.mkdir(parents=True, exist_ok=True)
    v = run_script("validators/validate", ["--task", task_id, "--continuation-id", cid], cwd=test_project_dir.tmp_path)
    assert v.returncode != 0  # expected missing reports
    assert "bundle-approved.json" in (v.stdout + v.stderr)


@pytest.mark.fast
def test_session_task_list_tracking(test_project_dir: TestProjectDir):
    ...
    for num, wave, slug in tasks:
        tid = f"{num}-{wave}-{slug}"
        assert_command_success(
            run_script("tasks/new", ["--id", num, "--wave", wave, "--slug", slug, "--session", session_id], cwd=test_project_dir.tmp_path)
        )
        assert_command_success(
            run_script("tasks/claim", [tid, "--session", session_id], cwd=test_project_dir.tmp_path)
        )
...
@pytest.mark.fast
def test_session_duration_tracking(test_project_dir: TestProjectDir):
    ...
    session_path_validated = test_project_dir.project_root / "sessions" / "validated" / f"{session_id}.json"
...
def test_complete_tracking_workflow(test_project_dir: TestProjectDir):
    ...
    assert_command_success(
        run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug, "--session", session_id], cwd=test_project_dir.tmp_path)
    )
    assert_command_success(
        run_script("tasks/claim", [task_id, "--session", session_id], cwd=test_project_dir.tmp_path)
    )

    session_wip_path = test_project_dir.project_root / "sessions" / "wip" / f"{session_id}.json"
    ...
    assert_command_success(
        run_script("session", ["remove", session_id, "task", task_id], cwd=test_project_dir.tmp_path)
    )
    ...
    validated_path = test_project_dir.project_root / "sessions" / "validated" / f"{session_id}.json"
...
@pytest.mark.fast
def test_qa_lifecycle_via_promote(test_project_dir: TestProjectDir):
    ...
    assert_command_success(run_script("tasks/new", ["--id", "170", "--wave", "wave1", "--slug", "qa-promote", "--session", session_id], cwd=test_project_dir.tmp_path))
    ...
    res_fail = run_script("qa/promote", ["--task", task_id, "--to", "todo", "--session", session_id], cwd=test_project_dir.tmp_path)
    assert res_fail.returncode != 0
    # Place task under tasks/done/ without using guard CLI (test setup only)
    todo_path = test_project_dir.project_root / "sessions" / "wip" / session_id / "tasks" / "todo" / f"{task_id}.md"
    done_path = test_project_dir.project_root / "sessions" / "wip" / session_id / "tasks" / "done" / f"{task_id}.md"
    ...
    assert_file_exists(test_project_dir.project_root / "sessions" / "wip" / session_id / "qa" / "todo" / f"{task_id}-qa.md")

    # todo → wip
    assert_command_success(run_script("qa/promote", ["--task", task_id, "--to", "wip", "--session", session_id], cwd=test_project_dir.tmp_path))
    assert_file_exists(test_project_dir.project_root / "sessions" / "wip" / session_id / "qa" / "wip" / f"{task_id}-qa.md")
    ...
    res_manual = run_script("qa/promote", ["--task", task_id, "--to", "done", "--session", session_id], cwd=test_project_dir.tmp_path)
    ...
    res_validate = run_script("validators/validate", ["--task", task_id], cwd=test_project_dir.tmp_path)
    ...
    assert_command_success(run_script("validators/validate", ["--task", task_id], cwd=test_project_dir.tmp_path))
    assert_command_success(run_script("qa/promote", ["--task", task_id, "--to", "done", "--session", session_id], cwd=test_project_dir.tmp_path))
    assert_file_exists(test_project_dir.project_root / "sessions" / "wip" / session_id / "qa" / "done" / f"{task_id}-qa.md")


@pytest.mark.fast
def test_session_complete_fail_closed_when_scope_not_ready(test_project_dir: TestProjectDir):
    ...
    assert_command_success(run_script("session", ["new", "--owner", "test", "--session-id", session_id, "--mode", "start"], cwd=test_project_dir.tmp_path))
    assert_command_success(run_script("tasks/new", ["--id", "180", "--wave", "wave1", "--slug", "incomplete", "--session", session_id], cwd=test_project_dir.tmp_path))
    # Attempt to complete should fail because scope not validated
    result = run_script("session", ["complete", session_id], cwd=test_project_dir.tmp_path)
