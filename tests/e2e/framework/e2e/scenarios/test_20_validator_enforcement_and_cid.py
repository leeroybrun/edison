...
@pytest.mark.fast
def test_specialized_blocking_database_and_testing_enforced(test_project_dir: TestProjectDir):
    ...
    res_fail = run_script("validators/validate", ["--task", task_id], cwd=test_project_dir.tmp_path)
    ...
    res_ok = run_script("validators/validate", ["--task", task_id], cwd=test_project_dir.tmp_path)
...
def test_run_wave_plumbs_continuation_id(test_project_dir: TestProjectDir):
    ...
    assert_command_success(run_script("tasks/new", ["--id", task_num, "--wave", wave, "--slug", slug, "--session", session_id], cwd=test_project_dir.tmp_path))
