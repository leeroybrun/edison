...
def _init_git_repo(repo_root: Path, base_branch: str = "main") -> None:
    cp = _git(repo_root, "init", "-b", base_branch)
...
def test_base_branch_loaded_from_manifest(test_project_dir: TestProjectDir, tmp_path: Path):
    """session new should respect worktrees.baseBranch from manifest."""
    # Patch manifest baseBranch to 'develop'
    manifest_path = test_project_dir.agents_root / "manifest.json"
    ...
    _init_git_repo(test_project_dir.tmp_path, base_branch="develop")
    ...
    sess_json = test_project_dir.get_session_json(sid)
    assert sess_json["git"]["baseBranch"] == "develop"


@pytest.mark.worktree
@pytest.mark.requires_git
def test_duplicate_worktree_detection_prefers_existing_path(test_project_dir: TestProjectDir):
    ...
    existing_dir = test_project_dir.tmp_path / "custom-wt" / sid
    existing_dir.parent.mkdir(parents=True, exist_ok=True)
    cp = _git(test_project_dir.tmp_path, "worktree", "add", "-b", branch, str(existing_dir), "main")
    if cp.returncode != 0:
...
@pytest.mark.fast
def test_archival_listing_sorted_by_mtime(test_project_dir: TestProjectDir):
    ...
    archive_root = test_project_dir.tmp_path / ".worktrees" / "_archived"
    archive_root_parent = test_project_dir.tmp_path.parent / ".worktrees" / "_archived"
    ...
    status = run_script("session", ["status", sid], cwd=test_project_dir.tmp_path)
