...
def _init_repo_without_touching_gpgsign(repo_root: Path, with_initial_commit: bool = True) -> None:
    ...
    if with_initial_commit:
        ...
        cp = _run_git(repo_root, "-c", "commit.gpgsign=false", "commit", "-m", "Initial commit")
        if cp.returncode != 0:
            raise RuntimeError(f"git commit failed: {cp.stderr}")


def _local_config_has_gpgsign_set(cwd: Path) -> bool:
    probe = _run_git(cwd, "config", "--local", "--get", "commit.gpgsign")
...
def test_91_no_repo_level_gpgsign_change(project_dir: TestProjectDir):
    ...
    _init_repo_without_touching_gpgsign(project_dir.tmp_path, with_initial_commit=True)

    # Sanity: starting state should NOT have commit.gpgsign set
    assert not _local_config_has_gpgsign_set(project_dir.tmp_path)
    ...
    sess = project_dir.get_session_json(session_id)
...
@pytest.mark.requires_git
@pytest.mark.edge_case
def test_92_worktree_failure_halts_and_does_not_claim_metadata(project_dir: TestProjectDir):
    ...
    _init_repo_without_touching_gpgsign(project_dir.tmp_path, with_initial_commit=False)
    ...
    path = project_dir.get_session_path(session_id)
