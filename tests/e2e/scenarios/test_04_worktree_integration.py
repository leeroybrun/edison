...
def _init_git_repo(repo_root: Path) -> None:
    ...
    cp = _run_git(repo_root, "init", "-b", "main")
...
def _git_changed_files(cwd: Path, base_branch: str = "main") -> list[str]:
    ...
    cp = _run_git(cwd, "diff", "--name-only", base_branch)
    if cp.returncode != 0:
...
def _git_worktree_paths(repo_root: Path) -> list[Path]:
    ...
    cp = _run_git(repo_root, "worktree", "list", "--porcelain")
    if cp.returncode != 0:
...
@pytest.mark.fast
def test_create_worktree_for_session(project_dir: TestProjectDir):
    ...
    session_path = project_dir.get_session_path(session_id)
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_branch_isolation(project_dir: TestProjectDir):
    ...
    git_meta = project_dir.get_session_json(session_id)["git"]
    ...
    assert in_wt.exists()
    assert not (project_dir.tmp_path / "test.txt").exists()


@pytest.mark.worktree
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_diff_detection(project_dir: TestProjectDir):
    ...
    worktree_path = Path(project_dir.get_session_json(session_id)["git"]["worktreePath"])
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_session_worktree_integration(project_dir: TestProjectDir):
    ...
    session_data = project_dir.get_session_json(session_id)
    git_meta = session_data["git"]
...
def test_context7_cross_check_with_git_diff(project_dir: TestProjectDir):
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_file_extension_detection(project_dir: TestProjectDir):
...
@pytest.mark.requires_git
@pytest.mark.slow
def test_worktree_multiple_commits(project_dir: TestProjectDir):
...
def test_worktree_no_changes(project_dir: TestProjectDir):
...
@pytest.mark.integration
@pytest.mark.slow
def test_worktree_full_workflow(project_dir: TestProjectDir):
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_list_all(project_dir: TestProjectDir):
    ...
    paths = _git_worktree_paths(project_dir.tmp_path)
...
@pytest.mark.requires_git
@pytest.mark.edge_case
def test_worktree_branch_name_validation(project_dir: TestProjectDir):
    ...
    data = project_dir.get_session_json(session_id)
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_base_branch_tracking(project_dir: TestProjectDir):
...
@pytest.mark.requires_git
@pytest.mark.integration
def test_worktree_concurrent_sessions(project_dir: TestProjectDir):
    ...
    for i in range(3):
        ...
        sessions.append(sid)
        wt = Path(project_dir.get_session_json(sid)["git"]["worktreePath"])
        p = wt / f"file-{i}.txt"
        p.write_text(f"Content {i}\n")
        _commit_all(wt, f"Add file-{i}")

    for sid in sessions:
        project_dir.assert_session_exists(sid)
        wt = Path(project_dir.get_session_json(sid)["git"]["worktreePath"])
        assert_directory_exists(wt)
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_detect_react_import(project_dir: TestProjectDir):
    ...
    wt = Path(project_dir.get_session_json(session_id)["git"]["worktreePath"])
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_detect_zod_import(project_dir: TestProjectDir):
...
@pytest.mark.requires_git
@pytest.mark.fast
def test_worktree_detect_prisma_schema(project_dir: TestProjectDir):
...
@pytest.mark.fast
def test_cross_session_isolation_enforced(project_dir: TestProjectDir):
