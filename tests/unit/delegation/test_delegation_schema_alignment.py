...
class TestDelegationSchemaAlignment(unittest.TestCase):
    def test_single_canonical_schema_file(self) -> None:
        # There must be exactly one delegation-config.schema.json and it must be at the canonical bundled path
        ...
        canonical = get_data_path("schemas", "config/delegation-config.schema.json")
    ...
    def test_schema_enum_excludes_none(self) -> None:
        ...
        schema_path = get_data_path("schemas", "config/delegation-config.schema.json")
    ...
    def test_config_points_to_canonical_schema_and_validates(self) -> None:
        ...
        cfg_path = get_data_path("delegation", "config.json")
