...
from edison.core.composition import AgentRegistry, generate_canonical_entry
...
def test_available_agents_file_written_to_generated_dir(tmp_path: Path) -> None:
    ...
    assert result_path.exists(), "AVAILABLE_AGENTS.md must be written to .edison/_generated"
...
def test_generation_header_contains_iso_timestamp(tmp_path: Path) -> None:
    ...
    generator = AgentRosterGenerator(project_root=tmp_path)
    result_path = generator.write(output_dir)
    content = _read_file(result_path)

    assert "GENERATED BY EDISON - DO NOT EDIT" in content
...
def test_agent_table_includes_all_registry_agents(tmp_path: Path) -> None:
    ...
    generator = AgentRosterGenerator(project_root=tmp_path)
    result_path = generator.write(output_dir)
    table_names = set(_table_names(_read_file(result_path)))
...
def test_generator_uses_registry_not_hardcoded_names(tmp_path: Path) -> None:
    ...
    generator = AgentRosterGenerator(project_root=repo_root)
    result_path = generator.write(output_dir)
