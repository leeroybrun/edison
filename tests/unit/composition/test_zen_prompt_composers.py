...
def _write_core_agent(root: Path, name: str) -> Path:
...
def _write_validator_spec(root: Path, validator_id: str, role: str) -> Path:
    """Create a minimal validator spec."""
    validator_dir = root / ".edison" / "core" / "validators" / "global"
    validator_dir.mkdir(parents=True, exist_ok=True)
    path = validator_dir / f"{role}-core.md"
...
def _write_config(root: Path, validators_roster: list = None) -> Path:
    """Write minimal config.yml with validator roster."""
    import yaml
    config_dir = root / ".edison"
...
def test_compose_agent_zen_prompt_contains_agent_content(isolated_project_env: Path) -> None:
    ...
    _write_core_agent(isolated_project_env, agent_name)
    _write_config(isolated_project_env)

    # When: Composing Zen prompt for the agent
    result = compose_agent_zen_prompt(agent_name, repo_root=isolated_project_env)
...
def test_compose_validator_zen_prompt_contains_validator_content(isolated_project_env: Path) -> None:
    ...
    validator_name = "codex-global"
    _write_validator_spec(isolated_project_env, validator_name, "codex")
    _write_config(isolated_project_env, validators_roster=[
        {"id": "codex-global", "name": "Codex Global Validator"}
    ])

    # When: Composing Zen prompt for the validator
    result = compose_validator_zen_prompt(validator_name, repo_root=isolated_project_env)
    ...
    assert "codex" in result.lower() or "Codex" in result, \
...
def test_compose_zen_prompt_dispatches_to_agent(isolated_project_env: Path) -> None:
    ...
    result = compose_zen_prompt(agent_name, repo_root=isolated_project_env)
...
def test_compose_zen_prompt_dispatches_to_validator(isolated_project_env: Path) -> None:
    ...
    # When: Calling the dispatcher
    result = compose_zen_prompt(validator_name, repo_root=isolated_project_env)
...
def test_compose_zen_prompt_raises_for_unknown_name(isolated_project_env: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Unknown agent or validator"):
        compose_zen_prompt(unknown_name, repo_root=isolated_project_env)
...
def test_agent_registry_has_exists_method(isolated_project_env: Path) -> None:
    ...
    _write_core_agent(isolated_project_env, "api-builder")
    registry = AgentRegistry(repo_root=isolated_project_env)
...
def test_validator_registry_has_exists_method(isolated_project_env: Path) -> None:
    ...
    _write_validator_spec(isolated_project_env, "codex-global", "codex")
    _write_config(isolated_project_env, validators_roster=[
        {"id": "codex-global", "name": "Codex Global Validator"}
    ])
    registry = ValidatorRegistry(repo_root=isolated_project_env)
    ...
    assert registry.exists("codex-global") is True, "Should find existing validator"
...
def test_validator_registry_has_get_method(isolated_project_env: Path) -> None:
    ...
    validator = registry.get("codex-global")
