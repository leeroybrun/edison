...
@pytest.mark.session
@pytest.mark.task
def test_tasks_split_creates_children_and_qas(project_dir: TestProjectDir):
    session_id = "split-session"
    # Create session
    res = run_script("session", ["new", "--owner", session_id, "--session-id", session_id, "--mode", "start"], cwd=project_dir.tmp_path)
    ...
    res_new = run_script("tasks/new", ["--id", parent_num, "--wave", wave, "--slug", parent_slug, "--session", session_id], cwd=project_dir.tmp_path)
    ...
    for cid in ids:
        task_path = project_dir.project_root / "sessions" / "wip" / session_id / "tasks" / "wip" / f"{cid}.md"
        assert task_path.exists(), f"missing child task: {task_path}"
        qa_path = project_dir.project_root / "sessions" / "wip" / session_id / "qa" / "waiting" / f"{cid}-qa.md"
