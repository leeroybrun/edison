...
class TestTaskIndexPerformance:
    ...
    def test_empty_project_scan_is_fast(self, tmp_path: Path):
        """Verify scanning an empty project is fast (<50ms)."""
        ...
        assert elapsed < 0.05, f"Empty scan took {elapsed:.3f}s, expected <50ms"
        assert results == []

    def test_small_project_scan_performance(self, tmp_path: Path):
        """Verify scanning a small project (10 tasks) is fast (<100ms)."""
...
""")
        ...
        assert elapsed < 0.1, f"Small project scan took {elapsed:.3f}s, expected <100ms"
        assert len(results) == 10

    def test_session_query_performance(self, tmp_path: Path):
        """Verify session query is fast (<100ms)."""
        # Create project structure matching TaskConfig (.project/tasks)
        tasks_dir = tmp_path / ".project" / "tasks" / "wip"
        tasks_dir.mkdir(parents=True)
        (tmp_path / ".project" / "sessions").mkdir(parents=True)
...
""")
        ...
        assert elapsed < 0.1, f"Session query took {elapsed:.3f}s, expected <100ms"
        assert len(tasks) == 10

    def test_task_graph_build_performance(self, tmp_path: Path):
        """Verify building task graph is fast (<200ms for 50 tasks)."""
        # Create project structure matching TaskConfig (.project/tasks)
...
""")
        ...
        assert elapsed < 0.2, f"Graph build took {elapsed:.3f}s, expected <200ms"
