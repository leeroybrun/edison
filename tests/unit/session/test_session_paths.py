...
def test_get_session_bases_without_session_id_finds_all_sessions(test_env):
    ...
    sessions_wip = test_env / ".project" / "sessions" / "wip"
...
def test_get_session_bases_with_session_id_finds_specific_session(test_env):
    ...
    repo = SessionRepository(project_root=test_env)
    session = Session.create("test-session-001", state="wip")
    repo.create(session)

    # Act
    bases = get_session_bases(session_id="test-session-001", project_root=test_env)
...
def test_get_session_bases_with_nonexistent_session_returns_candidates(test_env):
    ...
    # Act
    bases = get_session_bases(session_id="nonexistent", project_root=test_env)

    # Assert - still returns candidate paths (for creation)
    assert len(bases) > 0


def test_get_session_bases_handles_session_entity_data(test_env):
    ...
    # Act
    bases = get_session_bases(session_id="test-session-002", project_root=test_env)
...
def test_resolve_session_record_path_uses_session_repository(test_env):
...
def test_resolve_session_record_path_falls_back_to_search(test_env):
...
def test_resolve_session_record_path_defaults_to_wip(test_env):
...
def test_resolve_session_record_path_supports_task_record_type(test_env):
...
def test_resolve_session_record_path_ensures_session_id_in_path(test_env):
