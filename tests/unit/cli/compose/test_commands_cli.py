...
def test_compose_commands_cli_snippets_are_copy_pasteable(tmp_path: Path) -> None:
    ...
    session_start_text = (out_dir / "edison.session-start.md").read_text(encoding="utf-8")
    assert "edison list --type start --format detail" in session_start_text
    assert "edison read --type start START_<PROMPT_ID>" in session_start_text
