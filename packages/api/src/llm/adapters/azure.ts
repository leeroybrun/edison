...
export class AzureOpenAIAdapter implements LLMAdapter {
  ...
  async validateModel(params?: Partial<ModelParams>): Promise<void> {
    if (!this.modelId || !this.modelId.trim()) {
      throw new ValidationError('Model identifier is required for Azure OpenAI credentials.');
    }

    if (params?.temperature !== undefined && (params.temperature < 0 || params.temperature > 2)) {
      throw new ValidationError('Azure OpenAI temperature must be between 0 and 2.');
    }
  }
}
