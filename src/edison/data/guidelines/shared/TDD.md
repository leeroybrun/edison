...
- Use `edison task ready --run` to collect trusted evidence files; if TDD must be skipped, call `edison task ready --disable-tdd --reason "justification"` and record the rationale in the QA brief.
- Evidence files must be generated by `edison task ready --run` (trusted runner markers + timestamps). Manual fabrication is rejected.
...
1. RED: Write schema validation tests → verify fails
2. GREEN: Implement schema and migrations → verify passes
3. REFACTOR: Optimize indexes and constraints → verify still passes

Data Access Layer:
1. RED: Write repository/query tests → verify fails
2. GREEN: Implement data access methods → verify passes
3. REFACTOR: Optimize queries and add caching → verify still passes

Integration:
1. RED: Write integration tests with real database → verify fails
2. GREEN: Connect all layers → verify passes
3. REFACTOR: Clean up and optimize → verify still passes
