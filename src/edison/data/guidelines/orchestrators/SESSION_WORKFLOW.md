...
2. **Sub-agent will handle:**
   ...
   - ✅ Initializing evidence round: `edison qa round --new <task-id>` (or `edison evidence init <task-id>`)
...
1. **YOU must follow the agent constitution (run `edison read AGENTS --type constitutions`):**
   ...
   - Initialize evidence: `edison qa round --new <task-id>` (or `edison evidence init <task-id>`)
