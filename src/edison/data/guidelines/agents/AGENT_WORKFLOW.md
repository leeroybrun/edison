...
3. **Context refresh** – For any post-training packages in scope, run Context7 (resolve library ID via `mcp__context7__resolve_library_id`, then fetch docs via `mcp__context7__get_library_docs`) and drop `context7-<pkg>.txt` markers in the round evidence directory.
