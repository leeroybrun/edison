...
4. **TDD loop** – RED → GREEN → REFACTOR for each behavior. Keep tests isolated and deterministic. Keep coverage ≥{{config.quality.coverage.overall}}% overall / {{config.quality.coverage.changed}}% on changed/new files. For content-only Markdown/YAML/template edits, do not add brittle tests that pin wording or default config values; if you change executable behavior, TDD is required.
