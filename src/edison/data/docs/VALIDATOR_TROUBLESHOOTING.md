...
2. **Rebuild the bundle**: `edison qa bundle <task-id> [--session <session-id>]` and verify the bundle snippet included in the QA brief matches the current code state.
...
- **Prepare bundle**: `edison qa bundle <task-id> [--session <session-id>]` (must be fresh before every manual run).
- **Run all required validators**: `edison qa validate <task-id> [--session <session-id>] [--round <N>]` (round auto-increments when omitted). The command derives the validator list from YAML + the bundle; do not pass hardcoded IDs.
- **Re-run a subset**: `edison qa validate <task-id> --validators <id> --round <N>` to reproduce a specific failure without touching other reports. Use only after a full run produced evidence.
...
4. **Regenerate manifests**: Run `edison qa bundle <task-id>` to rebuild `_generated/validators/` and `AVAILABLE_VALIDATORS.md`, then commit the generated artifacts if your workflow requires.
5. **Run and verify**: Execute `edison qa validate <task-id> --validators <new-id>` for a focused check, confirm the report appears in the round directory, and document the new validator in the QA brief.
...
- **How do I see the exact roster the CLI will use?** Open `{{PROJECT_EDISON_DIR}}/_generated/AVAILABLE_VALIDATORS.md` after running `edison qa bundle <task-id>`.
