...
**BEFORE validating**, consult the `{{PACK_CONTEXT}}` section at the end of this document for framework-specific security patterns and best practices relevant to the technology stack in use.
...
**Check API Endpoints** (framework-specific patterns in `{{PACK_CONTEXT}}`):
...
1. ✅ **ALL** API endpoints require authentication (see framework-specific auth patterns in `{{PACK_CONTEXT}}`)
...
1. ✅ **Passwords**: NEVER stored in plain text
   ...
   - Verify framework's auth system is used, not custom implementation (see `{{PACK_CONTEXT}}`)
...
5. ✅ **HTTPS**: All production traffic uses HTTPS
   ...
   - Verify secure cookie flags (httpOnly, secure, sameSite) are set (see `{{PACK_CONTEXT}}`)
...
```typescript
// ✅ SAFE - Using ORM with parameterized queries (see {{PACK_CONTEXT}} for ORM-specific patterns)
...
// ✅ SAFE - Raw query with proper parameterization (framework-specific syntax in {{PACK_CONTEXT}})
const records = await database.raw(
...
1. ✅ **ORM/Query Builder used exclusively** (no raw queries) - check `{{PACK_CONTEXT}}` for database layer
...
1. ✅ **Authentication Flow**: Uses industry-standard authentication library (see `{{PACK_CONTEXT}}` for framework auth patterns)
2. ✅ **Session Management**: Secure session handling
   - Secure cookies with proper flags (httpOnly, secure, sameSite)
   - Session expiration and rotation
   - See `{{PACK_CONTEXT}}` for framework-specific session patterns
...
3. ✅ **CORS**: Properly configured (if applicable)
   ...
   // See {{PACK_CONTEXT}} for framework-specific CORS configuration
   ```

4. ✅ **Security Headers**: Set properly (see `{{PACK_CONTEXT}}` for framework defaults)
...
2. ✅ Check major framework and library packages are up-to-date (see `{{PACK_CONTEXT}}` for critical versions)
...
**Check Authentication System** (see `{{PACK_CONTEXT}}` for framework-specific auth patterns):
...
3. ✅ **Session Expiration**: Sessions expire after reasonable time period
   ...
   - Check `{{PACK_CONTEXT}}` for framework defaults
...
# See {{PACK_CONTEXT}} for framework-specific patterns to look for
```
...
2. ✅ **Input Validation**: ALL user input validated with schema validators (see `{{PACK_CONTEXT}}` for validation patterns)
   ```typescript
...
# Pattern depends on framework - see {{PACK_CONTEXT}}
...
# Check for redirect vulnerabilities (framework-specific patterns in {{PACK_CONTEXT}})
...
### Authentication Integration (see `{{PACK_CONTEXT}}`)
...
**Every API endpoint MUST follow this pattern** (see `{{PACK_CONTEXT}}` for framework-specific implementation):
...
### Database Security (see `{{PACK_CONTEXT}}`)
...
**Authentication Library Integration**: ✅ VERIFIED | ❌ NOT FOUND (see `{{PACK_CONTEXT}}` for expected patterns)
...
- **Always** consult `{{PACK_CONTEXT}}` for framework-specific security patterns
