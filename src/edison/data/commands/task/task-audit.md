...
domain: task
command: audit
short_desc: Audit `.project/tasks` for overlap, missing links, and duplicates
cli: edison task audit [--json]
args:
- name: --tasks-root
  description: Override tasks root (defaults to config-driven `.project/tasks`).
  required: false
- name: --threshold
  description: Similarity threshold override for duplicate detection.
  required: false
- name: --top-k
  description: Max duplicates per task.
  required: false
- name: --json
  description: Output as JSON.
  required: false
when_to_use: '- You want fast, deterministic signals about backlog collisions and
  drift
