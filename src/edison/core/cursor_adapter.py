...
Thin integration layer between the Edison composition system
(guidelines + rules + composed agents) and Cursor's configuration
surface:
...
from . import agents as _agents  # type: ignore
...
def _detect_repo_root(explicit_root: Optional[Path] = None) -> Path:
    ...
    canonical PathResolver so CursorAdapter shares the same semantics
