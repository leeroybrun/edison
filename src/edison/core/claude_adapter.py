...
"""
Claude Code Adapter for Edison Composition System.

This module bridges Edison's composed outputs under
``<project_config_dir>/_generated`` with Claude Code's ``.claude/``
configuration directory.

Responsibilities:
  - Validate Claude Code directory structure
  - Convert Edison-composed agent Markdown into Claude agent files
  - Sync orchestrator guide into ``.claude/CLAUDE.md``
  - Generate ``.claude/config.json`` with active agent roster

All filesystem interactions are real (no mocks) and respect the
AGENTS_PROJECT_ROOT / PathResolver semantics so tests can run against
isolated project roots without touching the real repository.
"""
...
try:  # Optional: expected to be available in Edison core env
    import jsonschema  # type: ignore
except Exception:  # pragma: no cover - surfaced via ConfigManager / doctor script
...
class ClaudeCodeAdapter:
    ...
    def _schemas_dir(self) -> Path:
        """Return core schemas directory for the current project root."""
        return self.repo_root / ".edison" / "core" / "schemas"

    def _load_schema(self, name: str) -> Dict[str, Any]:
        """Load a JSON schema by file name from the core schemas directory."""
    ...
    def _validate_agent_config(self, agent_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        schema = self._load_schema("claude-agent-config.schema.json")
        if not schema or jsonschema is None:  # type: ignore[truthy-function]
            return config
    ...
    def _validate_agent_payload(self, agent_name: str, payload: Dict[str, Any]) -> None:
        ...
        schema = self._load_schema("claude-agent.schema.json")
        if not schema or jsonschema is None:  # type: ignore[truthy-function]
            return
...
__all__ = ["ClaudeCodeAdapter", "ClaudeAdapterError", "EdisonAgentSections"]
