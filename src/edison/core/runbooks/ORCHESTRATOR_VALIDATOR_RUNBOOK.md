...
> Purpose: step-by-step guide for orchestrators to launch validators, read results, and drive rejection/escalation loops without hardcoded rosters. The active validator list, models, and trigger patterns are always in the dynamic roster at `AVAILABLE_VALIDATORS.md` (generated under `.edison/_generated/`).
...
- `bundle-approved.json` is the canonical summary:
...
- **Anti-patterns:** hardcoding validator counts/names, skipping the bundle step, rerunning in the same round and overwriting evidence, promoting QA without `bundle-approved.json`, or ignoring specialized validators that triggers require.
